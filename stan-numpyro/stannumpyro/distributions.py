...
categorical_logit_glm_lpmf = lambda y, x, alpha, beta: _unwrap(
    _lpmf(categorical_logit_glm)(y - 1, x, alpha, beta)
)
categorical_logit_glm_lupmf = lambda y, x, alpha, beta: _unwrap(
    _lupmf(categorical_logit_glm)(y - 1, x, alpha, beta)
)
...
ordered_logistic_glm_lpmf = lambda y, x, beta, c: _unwrap(
    _lpmf(ordered_logistic_glm)(y - 1, x, beta, c)
)
ordered_logistic_glm_lupmf = lambda y, x, beta, c: _unwrap(
    _lupmf(ordered_logistic_glm)(y - 1, x, beta, c)
)
...
multi_normal_cholesky = _distrib(
    lambda mu, l: d.MultivariateNormal(mu, tmatmul(l, ttranspose(l))), 2, dtype_float
)
